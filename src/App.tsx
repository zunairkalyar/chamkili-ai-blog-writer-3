      ...
      <main className="container mx-auto px-4 py-8 md:py-12">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
          ...
          <div className="lg:col-span-1 lg:sticky lg:top-24 space-y-6">
            <div className="bg-white rounded-2xl shadow-lg p-6 border border-gray-200">
              ...
              <div className="space-y-4">
                ...
                <button onClick={handleGenerate} disabled={mainButtonDisabled || (appMode === 'campaign' && !campaignGoal.trim())} className="w-full flex items-center justify-center gap-2 bg-[#D18F70] text-white font-bold py-3 px-4 rounded-lg hover:bg-[#C57F5D] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#C57F5D] disabled:bg-gray-400 disabled:cursor-not-allowed transition-all duration-300 transform hover:scale-[1.02]">
                  {(isGenerating || isGeneratingOutline) ? (<><svg className="animate-spin -ml-1 mr-3 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>Generating Content...</>) 
                  : (<>{appMode === 'blog' ? <SparkleIcon className="w-5 h-5" /> : <MegaphoneIcon className="w-5 h-5"/>}Generate Content</>)}
                </button>
              </div>
            </div>
            ...
            <SidebarPanel id="linking" title="Internal Linking" icon={<LinkIcon className="w-6 h-6"/>}>
                <InternalLinker 
                    url={sitemapUrl}
                    setUrl={setSitemapUrl}
                    links={internalLinks}
                    isLoading={isParsingSitemap}
                    error={sitemapError}
                    onFetch={handleFetchSitemap}
                />
            </SidebarPanel>
          ...
          <div className="lg:col-span-2">
            ...
            {(appState === 'ideation' && !isAutopilotActive) && (
                <div className="bg-white rounded-2xl shadow-lg border border-gray-200 min-h-[60vh] flex items-center justify-center text-center text-gray-400 p-8">
                  <div>
                    ...
                    <p className="text-sm">Start by entering a {appMode === 'blog' ? 'title' : 'goal'} and clicking "Generate Content".</p>
                  </div>
                </div>
            )}
            ...
            {(appState === 'generated' || (isAutopilotActive && contentBlocks.length > 0)) && (
              <>
                 {appMode === 'blog' ? (
                     <div onMouseUp={handleTextSelection} className="bg-white rounded-2xl shadow-lg border border-gray-200 min-h-[60vh]">
                        <BlogPostDisplay
                            ...
                            isLoading={isGenerating || isGeneratingOutline || (isAutopilotActive && autopilotStatus === 'working')}
