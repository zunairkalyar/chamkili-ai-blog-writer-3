    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-xl p-8 m-4 max-w-2xl w-full" onClick={e => e.stopPropagation()}>
        <div className="flex justify-between items-start mb-6">
            <div>
                <h2 className="text-2xl font-bold font-serif text-brand-primary flex items-center gap-3">
            ...
            <div className="space-y-3">
                <h3 className="font-semibold text-gray-800">1. Provide a Sample</h3>
                 <div className="flex border-b border-gray-200">
                    <button onClick={() => setAnalysisType('text')} className={`px-4 py-2 text-sm font-medium ${analysisType === 'text' ? 'border-b-2 border-brand-primary text-brand-primary' : 'text-gray-500 hover:bg-gray-100'}`}>From Text</button>
                    <button onClick={() => setAnalysisType('url')} className={`px-4 py-2 text-sm font-medium ${analysisType === 'url' ? 'border-b-2 border-brand-primary text-brand-primary' : 'text-gray-500 hover:bg-gray-100'}`}>From URL</button>
                </div>
                ...
                {analysisType === 'text' ? (
                     <div className="space-y-2 pt-2">
                        ...
                        <textarea
                            ...
                            rows={10}
                            className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-primary text-sm"
                ...
                <button
                    ...
                    className="w-full flex items-center justify-center gap-2 bg-brand-secondary text-white font-bold py-2.5 px-4 rounded-lg hover:bg-brand-primary disabled:bg-gray-400"
                >
                     {isAnalyzing ? (
