...
import { OutlineBlock } from '../services/gemini';
    ...
    return (
        <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-6 md:p-8 space-y-6">
            <div className="text-center">
                <SitemapIcon className="w-12 h-12 mx-auto text-brand-primary mb-2" />
                <h2 className="text-2xl font-bold font-serif text-brand-text-dark">Content Outline</h2>
                           ...
                           {isEditing && editCache ? (
                             <div className="space-y-2">
                                <input 
                                   ...
                                   className="w-full p-2 border border-gray-300 rounded-md font-semibold text-lg text-brand-primary focus:ring-2 focus:ring-brand-primary"
                                />
                                <textarea 
                                   value={editCache.keyPoints} 
                                   onChange={(e) => handleUpdateCache('keyPoints', e.target.value)}
                                   rows={4}
                                   className="w-full p-2 border border-gray-300 rounded-md text-gray-600 prose prose-sm focus:ring-2 focus:ring-brand-primary"
                                />
                             ...
                             <div className="flex items-start justify-between gap-4">
                                <div className="flex-1">
                                    <h3 className="font-semibold text-lg text-brand-primary">{item.heading}</h3>
                                    {isRegenerating ? (
