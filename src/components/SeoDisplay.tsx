...
export const SeoDisplay: React.FC<SeoDisplayProps> = ({
  ...
  if (isLoading) {
    return (
      <div className="mt-8 bg-white rounded-2xl shadow-lg p-6 border border-gray-200 flex items-center justify-center text-center text-gray-500">
        <svg className="animate-spin -ml-1 mr-3 h-5 w-5 text-brand-primary" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
  ...
  return (
    <div className="mt-8 bg-white rounded-2xl shadow-lg p-6 border border-gray-200">
      <h3 className="text-xl font-serif font-bold text-brand-primary mb-4 flex items-center gap-3">
        <SeoIcon className="w-6 h-6"/> SEO & Additional Content
      </h3>
      <div className="space-y-6">
        
        {seoData.keyTakeaways?.length > 0 && (
          <div>
            ...
            {seoData.metaTitles.map((title, index) => (
                <label key={index} htmlFor={`title-${index}`} className="flex items-start gap-3 p-2 rounded-md hover:bg-gray-100 cursor-pointer">
                    <input
                        ...
                        className="mt-1 h-4 w-4 text-brand-primary border-gray-300 focus:ring-brand-primary"
                    />
