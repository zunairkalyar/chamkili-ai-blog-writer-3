...
import { ImageState, ContentBlock } from '../types';
...
export const BlogPostDisplay: React.FC<BlogPostDisplayProps> = ({ blocks, imageStates, isLoading, onRetryImage, onUpdateImagePrompt }) => {
  ...
  const handleCopy = (type: 'html' | 'markdown') => {
    ...
    const contentToCopy = type === 'html'
      ? convertBlocksToHtml(blocks, imageStates)
      : convertBlocksToMarkdown(blocks, imageStates);
  ...
  return (
    <div className="relative p-6 md:p-10 min-h-[400px]">
      {hasContent && !isLoading && (
        <div className="absolute top-4 right-4 flex items-center gap-2 z-10">
            <button
              ...
              className="flex items-center gap-2 px-3 py-1.5 bg-white border border-gray-200 rounded-lg text-sm text-gray-600 hover:bg-gray-100 hover:text-gray-800 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-white focus:ring-brand-primary transition-all disabled:opacity-60"
      ...
      {isLoading && blocks.length === 0 ? (
        ...
        <div className="prose prose-lg max-w-none prose-h1:text-brand-primary prose-h2:text-brand-text-dark prose-p:text-gray-600 prose-ul:text-gray-600 pt-8">
          {MemoizedContent}
